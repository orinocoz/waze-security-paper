...
class PipelinedRequester
{
        ...
        throws IOException
    {
        if (connection == null || !connection.isOpen())
        {
            ...
            java.net.Socket socket1 = socketFactory.connectSocket(socket, host.getHostName(), host.getPort(), null, 0, basichttpparams);
            connection.bind(socket1, basichttpparams);
        }
    }
...
label0:
            {
                if (httpconnectionmetrics.getResponseCount() < httpconnectionmetrics.getRequestCount())
                {
                    ...
                    if (aheader != null)
                    {
                        ...
                        for (int j = 0; j < i; j++)
                        {
                            if ("close".equalsIgnoreCase(aheader[j].getValue()))
                            {
                                callbacks.pipelineModeChanged(false);
                                canPipeline = false;
                            }
